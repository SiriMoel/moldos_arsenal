    ...
    for i,target_id in ipairs( targets ) do
        ...
        if EntityGetName(target_id) == "$animal_worm_big" then
            if ( EntityHasTag( target_id, "kupoli_worm_enhanced" ) == false ) then
                ...
                {
                    script_death = "mods/moldos_arsenal/files/entities/projectiles/worm_enhancer/enhanced_death.lua",
                    execute_every_n_frame = "-1",
                } )
    
                local effect_id = EntityLoad("mods/moldos_arsenal/files/entities/projectiles/worm_enhancer/enhanced_particles.xml", x, y)
